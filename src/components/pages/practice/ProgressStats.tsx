"use client";
import { Progress } from "@/components/ui/progress";
import { usePracticeSession } from "@/hooks/usePracticeSession";
import { Languages } from "lucide-react";

export const ProgressStats = () => {
  const { currentIndex, totalWords } = usePracticeSession("recall");

  return (
    <div className="mt-8 w-3/4 md:w-full md:max-w-md bg-sky-900 dark:bg-indigo-950/50 rounded-xl shadow p-4">
      <h3 className="text-sm font-semibold text-white mb-2 flex items-center gap-1">
        <Languages className="w-4 h-4 text-indigo-500" /> Current session
        progress
      </h3>
      <Progress value={((currentIndex + 1) * 100) / totalWords} />
      <p className="mt-2 text-sm text-gray-200 dark:text-gray-400">
        {currentIndex + 1} / {totalWords} words practiced
      </p>
    </div>
  );
};
